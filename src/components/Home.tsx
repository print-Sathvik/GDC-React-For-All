...
export default function Home(props: { openFormCB: (id: number) => void }) {
  ...
  const deleteForm: (id: number) => void = (id: number) => {
    ...
    console.log(JSON.stringify(savedFormsState));
  };

  //The form should be cleared from local storage as sson as the state gets updated in the above deleteForm method
  useEffect(() => {
  ...
  return (
    ...
    <div className="divide-y-2">
      {savedFormsState?.map((form: formData) => (
        <div key={form.id} className="flex">
          <h2 className="py-4 px-2 flex-1">{form.title || "Untitled Form"}</h2>
          <button
            className="px-2 my-2"
