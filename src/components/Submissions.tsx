...
const fetchSubmissions = async (form_id: number, setSubmissionsCB:(value: allSubmissions) => void) => {
  try {
...
export default function Submissions(props: { id: number }) {
  const [submissions, setSubmissions] = useState<allSubmissions>({count: 0, results: []});
      ...
      <div className="divide-y-2">
        {submissions.results?.map((submission: Submission) => (
            <div key={submission.id} className="flex">
              ...
              <Link href={`/forms/${props.id}/submission/${submission.id}`} className="p-2 my-2 mr-2">
